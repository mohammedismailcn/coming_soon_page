...
const material = new THREE.ShaderMaterial({
  ...
  fragmentShader: `
    ...
    // --- Compact value noise (cheaper hash) ---
    float hash(vec2 p) {
    ...
    // 3-octave fbm — was 5, cuts per-call cost by 40%
    float fbm(vec2 p) {
      float v = 0.0, a = 0.5;
    ...
    // Single-warp flow fbm — was double-warp (5 fbm calls), now 2
    float flowFbm(vec2 p, float t) {
      vec2 warp = vec2(
        fbm(p * 1.3 + vec2(0.0, t * 0.14)),
        fbm(p * 1.3 + vec2(4.8, -t * 0.11))
      );
      return fbm(p + (warp - 0.5) * 0.8 + vec2(t * 0.05, 0.0));
    }

    float foamLine(vec2 p, float w) {
      return smoothstep(w, 0.0, abs(fbm(p) - 0.54));
    }
    ...
    void main() {
      ...
      // River coordinate frame + slow meander
      ...
      // Cheap fake normal: derive from body gradient using finite diff on fbm only
      // (no extra flowFbm calls — saves 4 × flowFbm = 12 fbm calls per pixel)
      ...
      float specular = pow(clamp(dot(normal, normalize(lDir + vec3(0.0, 0.0, 1.0))), 0.0, 1.0), 32.0) * 0.45;
      ...
      // Caustics — single fbm, cheap
      float caust = fbm(stream * vec2(13.0, 5.5) + offFast * 1.5);
      color += pow(smoothstep(0.56, 0.94, caust), 2.0) * 0.10 * vec3(0.6, 0.95, 1.0);

      // One foam layer (was two)
      float foam = foamLine(stream * vec2(7.5, 2.4) + offSlow * 1.1 + vec2(0.0, body * 2.2), 0.10);
      ...
      for (int i = 0; i < ${MAX_RIPPLES}; i++) {
        ...
        float fade = 1.0 - smoothstep(0.0, 2.4, age);
        float ring = exp(-pow((d - front) * 13.0, 2.0)) * fade;
        color += ring * 0.55 * vec3(0.72, 0.96, 1.0);
